import { getMonetaryValue, getUpdatedAveragePrice } from "./taxMathUtils";

/* eslint-disable @typescript-eslint/no-explicit-any */
describe("taxMathUtils", () => {
	describe("getUpdatedAveragePrice", () => {
		...
		test.each([
			...
			(position, averagePrice, positionIncrement, valueIncrement, updatedAveragePrice) => {
				expect(getUpdatedAveragePrice(position, averagePrice, positionIncrement, valueIncrement)).toBe(
					updatedAveragePrice,
				);
			},
		);
	...
	test.each([
		...
		expect(getMonetaryValue(value)).toBe(monetaryValue);
	});
});
