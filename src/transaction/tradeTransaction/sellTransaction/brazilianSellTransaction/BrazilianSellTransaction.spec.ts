...
import { BrazilianSellTransaction } from "./BrazilianSellTransaction";

describe(BrazilianSellTransaction.name, () => {
	const ANYTHING: any = undefined;

	it("should throw an error when price is negative", () => {
		expect(() => new BrazilianSellTransaction(ANYTHING, ANYTHING, -1, ANYTHING)).toThrow(
			`${TradeTransaction.name} price -1 must not be negative.`,
		);
	});

	it("should throw an error when quantity is not positive", () => {
		expect(() => new BrazilianSellTransaction(ANYTHING, ANYTHING, ANYTHING, 0)).toThrow(
			`${TradeTransaction.name} quantity 0 must be positive.`,
		);
		expect(() => new BrazilianSellTransaction(ANYTHING, ANYTHING, ANYTHING, -1)).toThrow(
	...
	it.each([
		...
		const transaction = new BrazilianSellTransaction(ANYTHING, ANYTHING, price, quantity);
