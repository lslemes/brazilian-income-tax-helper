...
import { InternationalBuyTransaction } from "./InternationalBuyTransaction";

describe(InternationalBuyTransaction.name, () => {
	const ANYTHING: any = undefined;

	it("should throw an error when price is negative", () => {
		expect(() => new InternationalBuyTransaction(ANYTHING, ANYTHING, -1, ANYTHING)).toThrow(
			`${TradeTransaction.name} price -1 must not be negative.`,
		);
	});

	it("should throw an error when quantity is not positive", () => {
		expect(() => new InternationalBuyTransaction(ANYTHING, ANYTHING, ANYTHING, 0)).toThrow(
			`${TradeTransaction.name} quantity 0 must be positive.`,
		);
		expect(() => new InternationalBuyTransaction(ANYTHING, ANYTHING, ANYTHING, -1)).toThrow(
	...
	it.each([
		...
		const transaction = new InternationalBuyTransaction(ANYTHING, ANYTHING, price, quantity);
